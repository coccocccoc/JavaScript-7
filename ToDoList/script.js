...
function addTodo() {
  ...
  checkbox.addEventListener('change', () => {
    ...
    countTodo(); // 체크박스의 상태가 변경되었을 때 count 다시 세기
  });


  const textInput = document.createElement('input');
  textInput.type = 'text';
  textInput.value = text; // 입력 필드에 꺼낸 값으로 초기화
  textInput.disabled = true; // 수정 불가, 읽기모드
  const removeBtn = document.createElement('button');
  removeBtn.textContent = '삭제';
  // 삭제버튼을 클릭하면 목록에서 해당 항목이 삭제됨
  // 단건 삭제
  removeBtn.addEventListener('click', () => {
    // button을 포함하고 있는 li 태그를 삭제
    li.remove();
    countTodo(); // 요소가 삭제됐을 때 다시 count 세기
  });
...
function deleteAll() {
  ...
  // querySelectorAll: 조건을 만족하는 모든 요소를 가져오가
  ...
  countTodo(); // todo가 삭제됐으면 다시 count 세기
}
